...
describe("ghostTrackAtCell — 非アクティブトラックのノートからトラックを特定", () => {
  // pitch = ROWS-1-row。row 60 → pitch 67
  const clipOf = (...notes) => ({ notes });
  const NG = (pitch, start, len = 1) => ({ pitch, start, len });

  it("そのセルを覆う非アクティブトラックの index を返す (アクティブは除外)", () => {
    const clips = [
      clipOf(), // 0 = アクティブ (空)
      clipOf(NG(ROWS - 1 - 60, 4, 2)), // 1 = row 60, col 4..5
      clipOf(),
      clipOf(),
    ];
    expect(ghostTrackAtCell(4, 60, 0, clips)).toBe(1); // 開始セル
    expect(ghostTrackAtCell(5, 60, 0, clips)).toBe(1); // len 内
    expect(ghostTrackAtCell(6, 60, 0, clips)).toBe(-1); // len 外
    expect(ghostTrackAtCell(4, 61, 0, clips)).toBe(-1); // 別の行
  });

  it("アクティブトラックのノートは無視する (自トラックでは切り替えない)", () => {
    const clips = [clipOf(NG(ROWS - 1 - 60, 0, 1)), clipOf(), clipOf(), clipOf()];
    expect(ghostTrackAtCell(0, 60, 0, clips)).toBe(-1);
  });

  it("重なりは最前面 (最大 index) を優先する", () => {
    const clips = [
      clipOf(),
      clipOf(NG(ROWS - 1 - 60, 0, 4)), // 1
      clipOf(),
      clipOf(NG(ROWS - 1 - 60, 0, 4)), // 3 (前面)
    ];
    expect(ghostTrackAtCell(1, 60, 0, clips)).toBe(3);
  });

  it("どのトラックにも無ければ -1", () => {
    const clips = [clipOf(), clipOf(), clipOf(), clipOf()];
    expect(ghostTrackAtCell(10, 40, 0, clips)).toBe(-1);
  });
});
