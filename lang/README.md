...
| `canvas:` | `canvas: 1920x1080` | 出力解像度（外寸px。`1920 1080` でも可）。プレビューもこのアスペクト比 |
...
コアはこれらを**不透明データ**（`{ view, canvas, pixel, pad, fps, seed, period }`）として持つだけで
**ラスタライズ・適用しない**。既定値・範囲クランプ・実際の出力はホスト（TESSERA / `art_export`）の
責務。未指定は各ホストの既定（TESSERA は `dither(2)` / `1080x1080` / `pixel:8` / `pad:0` / `fps:20` / `seed:0` / `period:tau`）。
