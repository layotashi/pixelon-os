...
| `seed:` | `seed: 42` | 乱数シード（rnd/noise/fbm を振り直す） |
...
## 現状
**場 `f(x,y,t)`（値ブロック含む）の単一パラダイム**。SYNESTA との **場レンダラ共有（北極星 B）**
はピクセル方式（`core/field_render.js`）＋ ASCII（`core/ascii_art.js` の `renderAsciiLines`）まで完了。
