...
| `size:` | `size: 1920x1080` | 出力外寸（`1920 1080` でも可）。プレビューもこのアスペクト比 |
| `pixel:` | `pixel: 8` | 1 アートピクセル = N 出力px（粗さ＝チャンキーさ。方式 dither とは別物） |
| `pad:` | `pad: 64` | 額縁マット（出力px） |
| `fps:` | `fps: 20` | アニメ/書き出しのフレームレート |
| `seed:` | `seed: 42` | 乱数シード（rnd/noise/fbm・状態場の初期化） |

コアはこれらを**不透明データ**（`{ view, size, pixel, pad, fps, seed }`）として持つだけで
**ラスタライズ・適用しない**。既定値・範囲クランプ・実際の出力はホスト（TESSERA / `art_export`）の
責務。未指定は各ホストの既定（TESSERA は `dither(2)` / `1080x1080` / `pixel:8` / `pad:0` / `fps:20` / `seed:0`）。
...
設定ディレクティブ**で指定し、画面のコントロールは「形式 + DL」のみ＝最小。プレビューは `size:` の
