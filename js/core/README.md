...
| ファイル        | 役割                                                             |
| --------------- | ---------------------------------------------------------------- |
| `gpu.js`        | 1-bit VRAM + 描画プリミティブ (pset/line/rect/blit 等)、autoScale、Canvas 転送 |
| `display_fx.js` | 表示エフェクト (Vignette + Diagonal scanline)、VRAM → RGBA 展開   |
| `dither.js`     | Bayer ordered dithering (RGBA → 1-bit)                           |
| `field_render.js` | スカラー場 (0..1) → 1-bit の共有レンダラ (dither/hatch/halftone/braille) |
...
| ファイル        | 役割                                                             |
| --------------- | ---------------------------------------------------------------- |
| `input.js`      | キーボード・マウス状態管理 + セマンティックイベントログ (click/drag/dblclick 等) |
...
| `defaults.js`   | 現在の全設定を「出荷時デフォルト」として書き出す (config.js へベイク運用) |

**音声・コーデック・書き出し**
| ファイル        | 役割                                                             |
| --------------- | ---------------------------------------------------------------- |
| `audio.js`      | Web Audio 基盤 (AudioContext / SynthChannel / SamplePlayer / SFX / 音楽ユーティリティ) |
